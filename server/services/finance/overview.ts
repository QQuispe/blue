...
export async function getOverviewData(userId: number): Promise<OverviewData> {
  const currencySettings = await getCurrencyFromSettings(userId)
  ...
  const cashFlowQuery = await pool.query(
    `SELECT 
     ...
     ORDER BY month DESC`,
    [userId, previousRange.startDate, currentRange.endDate]
  )

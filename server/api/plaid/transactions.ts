...
import { applyTransactionUpdates } from '~/server/db/queries/transactions'
...
export default defineEventHandler(async (event): Promise<SyncResponse> => {
  ...
  try {
    ...
    const syncData = await syncTransactionsFromPlaid(
      accessToken,
    ...
    const { updateItemSync } = await import('~/server/db/queries/items')

...
import { getUserWorkoutPlans, createWorkoutPlan } from '~/server/services/health/workoutPlans'
...
export default defineEventHandler(
  withErrorHandling(async (event): Promise<ApiSuccess<any>> => {
    ...
    if (method === 'GET') {
      const { activePlan, plans } = await getUserWorkoutPlans(user.id)
      return apiSuccess({ activePlan, plans })
    }

    if (method === 'POST') {
      const body = await readBody<any>(event)
      const plan = await createWorkoutPlan(user.id, body)

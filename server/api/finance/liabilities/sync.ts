...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    for (const liability of allLiabilities) {
      ...
      if (existingBill.rows.length > 0) {
        ...
        await pool.query(
          ...
          [user.id, liability.name, liability.minimumPayment, 'monthly', 
           liability.nextPaymentDueDate, 'plaid_liabilities', liability.accountId, liability.type]
        );
        addedCount++;
      }
    }
    ...
    serverLogger.api('GET', '/api/user/liabilities/sync', error.statusCode || 500, Date.now() - startTime);
    throw createError({

...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    serverLogger.api('POST', `/api/user/bills/suggestions/${suggestionId}/ignore`, 200, Date.now() - startTime, user.id);
    ...
    serverLogger.api('POST', `/api/user/bills/suggestions/${getRouterParam(event, 'id')}/ignore`, error.statusCode || 500, Date.now() - startTime);
    throw createError({
